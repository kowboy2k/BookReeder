...
- **Cadence du modèle par défaut** : durée d'un groupe = `base × nb mots` (cadence CONSTANTE
...
- **Titres / civilités** (`HONORIFIQUE`, `estHonorifique`) : M., MM., Mr, Mrs, Ms, Mme, Mlle,
  Dr, Pr, Me, Mgr, St… sont toujours groupés avec le nom propre qui suit (jamais séparés), et
  leur point d'abréviation n'est pas considéré comme une fin de phrase (`estDebutPhrase`).
